...
export default function DeleteMatchButton({ matchId }: { matchId: string }) {
  ...
  async function handleDelete() {
    ...
    // Delete match_players first (cascade should handle it, but explicit is safer)
    ...
    if (playersErr) {
      setError('Gagal menghapus data match.')
      setDeleting(false)
      return
    }
    ...
    if (matchErr) {
      setError('Gagal menghapus match.')
      setDeleting(false)
      return
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)' }}>
          <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-semibold">Hapus Match?</h3>
                <p className="text-slate-400 text-sm">Data match ini akan dihapus permanen.</p>
              </div>
            </div>
